...
import { studentMessages } from "../../data/Data";

const initialChats = {
  1: [
    { from: "tutor", text: "Hi! Ready for tomorrow's session?", time: "9:50 AM" },
    { from: "student", text: "Yes! Should I prepare anything?", time: "9:55 AM" },
    { from: "tutor", text: "See you tomorrow at 4 PM!", time: "10:00 AM" },
  ],
  2: [
    { from: "tutor", text: "I've uploaded the notes for today.", time: "8:00 AM" },
    { from: "student", text: "Thank you so much!", time: "8:05 AM" },
  ],
  3: [
    { from: "tutor", text: "Please review chapter 3 before our session.", time: "Yesterday" },
  ],
};

const S_Chat = () => {
  const [selected, setSelected] = useState(studentMessages[0]);
  ...
  const sendMessage = () => {
    ...
    setChats((p) => ({
      ...p,
      [selected.id]: [...(p[selected.id] || []), { from: "student", text: input.trim(), time: now }],
    }));
    setInput("");
  };
      ...
      <div className="flex h-full">
            ...
            {studentMessages.map((msg) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate">{msg.tutor}</p>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="px-5 py-3.5 border-b border-gray-100 flex items-center gap-3 bg-gradient-to-r from-purple-50 to-blue-50">
            ...
            <div>
              <p className="font-semibold text-gray-800 text-sm">{selected?.tutor}</p>
            ...
            {(chats[selected?.id] || []).map((msg, i) => (
              <div key={i} className={`flex ${msg.from === "student" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-xs lg:max-w-sm px-4 py-2.5 rounded-2xl text-sm ${
                  ...
                  <p>{msg.text}</p>
                  <p className={`text-xs mt-1 ${msg.from === "student" ? "text-white/60" : "text-gray-400"}`}>{msg.time}</p>
                </div>
              </div>
            ))}
            <div ref={bottomRef} />
